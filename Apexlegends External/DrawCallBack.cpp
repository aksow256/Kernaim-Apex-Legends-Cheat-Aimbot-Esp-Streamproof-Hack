...
void drawEntity() {
	...
	//drawList->AddCircle({ (float)CentWindow.x, (float)CentWindow.y }, appConfigs.ZiMiaoFanWei, ImColor({ 0x00, 0xff, 0xff }),50, 1.2f);
	...
	for (ApexEntity entity : apexEntityList) {
		...
		if (entity.type == 0) {
			if (entity.distance > 50.f)continue;
		...
		startFor:
			...
			if (appConfigs.FangKuang) {//方框
				...
				if (entity.distance < appConfigs.TouShiFanWei && appConfigs.GUGETOUSHI) {
	...
	float losInside = 0;
	Vec3D myLocal = {};
	readVec3D(_VIEWANGLE - 28, &myLocal);
	...
	if (appConfigs.ZiDongMiaoZhun) {
		if (GetAsyncKeyState(appConfigs.MiaoZhunReJian)) {
			if (aimEntity > 0) {
				aim = true;
				if (aimThreadStop) {
					aimThreadStop = false;
